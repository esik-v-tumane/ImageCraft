...
#pragma pack(push, 1) // Выключаем выравнивание структур для правильного чтения BMP
...
typedef struct {
    ...
    int32_t  height;              // Высота изображения в пикселях (положительное - снизу вверх)
    ...
    uint32_t image_size;          // Размер данных изображения в байтах (0 или реальный размер)
    ...
    uint32_t colors_used;         // Число используемых цветов в палитре (0 - нет палитры)
    uint32_t colors_important;    // Число важных цветов (0 - все важны)
} BMPInfoHeader;
...
typedef struct {
    ...
    RGBPixel** pixels;          // Двумерный массив пикселей [height][width]
} BMPImage;
...
bool bmp_is_valid_24bit(const char* filename);
